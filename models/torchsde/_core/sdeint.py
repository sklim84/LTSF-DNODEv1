...
def check_contract(sde, y0, ts, bm, method, adaptive, options, names, logqp):
    ...
    def _check_3d(name, shape):
        ...
        seq_sizes.append(shape[2])
        

    def _check_3d_or_4d(name, shape):
    ...
    if hasattr(sde, 'f'):
        ...
        _check_3d('Drift', f_drift_shape)
    if hasattr(sde, 'g'):
        has_g = True
        g_diffusion_shape = tuple(sde.g(ts[0], y0).size())
        _check_3d_or_4d('Diffusion', g_diffusion_shape)
    ...
    if hasattr(sde, 'g_prod'):
        ...
        _check_3d('Diffusion-vector product', g_prod_shape)
