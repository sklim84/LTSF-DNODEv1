...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.ode_func_R = ODEFunc(self.ode_hidden_size_R, self.num_ode_func_layers, act_func=self.act_func_ode_func)
    ...
    def forward(self, x, t):
        ...
        # Residual
        opt_R = {}
        opt_R.update({'method': self.ode_solver})
        opt_R.update({'h': self.ode_step_size_R})
        opt_R.update({'t0': 0.0})
        opt_R.update({'t1': 1.0})
        opt_R.update({'rtol': self.rtol_R})
        opt_R.update({'atol': self.atol_R})
        opt_R.update({'print_neval': False})  # nfe counts
        opt_R.update({'neval_max': 1000000})

        init_R = self.revin_R(init_R, 'norm')
        init_R = self.enc_R(init_R)
        init_R = (init_R, torch.zeros(init_R.size(0)).to(init_R), torch.zeros(init_R.size(0)).to(init_R))
        y_R, y_R_kinetic, y_R_jacobian = odeint(func=self.ode_func_R, y0=init_R, options=opt_R)
        y_R = self.dec_R(y_R)
        y_R = self.revin_R(y_R, 'denorm')
        reg_R = self.kinetic * y_R_kinetic.mean() + self.jacobian * y_R_jacobian.mean()

        y = y_T + y_S + y_R
        reg = reg_T + reg_S + reg_R
